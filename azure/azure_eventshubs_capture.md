...
## Event Hub Capture Default Format

**Event Hub Capture writes in AVRO format by default**, not JSONL. The captured files look like:
```
namespace.mongodb-changes.0.2025-01-15-14-30-45.avro
```

## My Code Has a Problem

In my updated code, I wrote:
```javascript
...
**This doesn't actually work!** I threw an error instead of implementing AVRO parsing.

## Two Solutions

### **Solution 1: Configure Event Hub Capture for JSON (Easier)**

Unfortunately, **Event Hub Capture doesn't natively support JSON output** - it only supports AVRO format. So this isn't actually possible.

### **Solution 2: Parse AVRO in Azure Function (Required)**

We need to actually parse the AVRO files. Let me fix the code:## Corrected Data Flow

### **Event Hub Capture Creates:**
- **Format**: AVRO files (not JSONL)
- **Files**: `namespace.mongodb-changes.0.2025-01-15-14-30-45.avro`
- **Content**: Binary AVRO with Event Hub metadata + MongoDB change stream JSON in the body

### **Azure Function Processes:**
1. **Reads AVRO files** from Event Hub Capture
2. **Parses AVRO** to extract individual events
3. **Transforms events** to clean format
4. **Writes JSONL files** to organized hierarchy for Snowflake

### **Final Output is JSONL:**
```
processed/mongodb-changes/inventory_db/products/2025/01/15/14-batch123.jsonl.gz
```
...
MongoDB Change Streams 
    ↓ (real-time, JSON)
Event Hub
    ↓ (automatic capture, AVRO format)
ADLS Gen2: Raw Capture Files
    ↓ (Timer Function processes AVRO → JSONL)
ADLS Gen2: Processed JSONL Files 
    ↓ (Snowpipe)
Snowflake
```
...
Even with AVRO parsing, Event Hub Capture is superior because:

✅ **Guaranteed Batch Windows**: Files appear exactly on schedule (hourly)
✅ **Zero Event Hub Client Code**: No connection management in your function
✅ **Azure-Managed Reliability**: Microsoft handles the capture infrastructure  
✅ **True Batch Semantics**: Clean hourly boundaries
✅ **Simplified Function Logic**: Just file processing, no event streaming

**Your function becomes a simple "file transformer"** rather than a complex "event stream processor."

The AVRO parsing adds a bit of complexity, but it's still much simpler and more reliable than managing Event Hub clients directly!
...
class EventHubCaptureProcessor {
  ...
  async parseAvroContent(content, context) {
    try {
      // Event Hub Capture uses a specific AVRO schema
      // The schema is embedded in the AVRO file header
      const decoder = avro.createFileDecoder(content);
      const events = [];
      
      for (const record of decoder) {
        // Event Hub Capture AVRO record structure:
        // {
        //   "SequenceNumber": long,
        //   "Offset": string, 
        ...
        let bodyContent;
        try {
          // Body is typically UTF-8 encoded JSON from MongoDB change stream
          bodyContent = JSON.parse(record.Body.toString('utf-8'));
        } catch (error) {
          context.log.warn(`Failed to parse body as JSON: ${error.message}`);
          bodyContent = record.Body.toString('utf-8');
        }
        
        events.push({
          sequenceNumber: record.SequenceNumber,
          offset: record.Offset,
          enqueuedTimeUtc: record.EnqueuedTimeUtc,
          systemProperties: record.SystemProperties,
          properties: record.Properties,
          body: bodyContent
        });
      }
      
      context.log(`Parsed ${events.length} events from AVRO file`);
      return events;
      
    } catch (error) {
      context.log.error(`AVRO parsing failed:`, error);
      return [];
    }
  }
