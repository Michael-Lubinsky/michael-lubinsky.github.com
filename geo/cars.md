...
SELECT DISTINCT
  ...
  LAST_VALUE(odometer) OVER (PARTITION BY car_id, trip_id ORDER BY time 
...
FROM (
  SELECT *,
    ROW_NUMBER() OVER (PARTITION BY car_id, trip_id ORDER BY time ASC) as rn_first,
    ROW_NUMBER() OVER (PARTITION BY car_id, trip_id ORDER BY time DESC) as rn_last
  FROM your_table_name
) ranked
...
SELECT 
  ...
  e.end_latitude
FROM start_points s
JOIN end_points e ON s.car_id = e.car_id AND s.trip_id = e.trip_id
ORDER BY s.car_id, s.trip_id;
```
